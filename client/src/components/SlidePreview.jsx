        ...
        <div className="p-8 bg-gradient-to-b from-white to-gray-50/30">
            ...
            <ul className="space-y-4 mb-10">
                {bullets && bullets.map((bullet, idx) => (
                    <li key={idx} className="flex gap-4 text-gray-700 text-sm leading-relaxed group/item hover:text-slate-900 transition-colors">
                        <span className="text-accent font-bold text-lg leading-none mt-0.5 transform transition-transform group-hover/item:scale-110">›</span>
            ...
            <div className="mt-4 border border-blue-100 bg-blue-50/50 rounded-xl p-5 flex items-start gap-5 transition-colors hover:bg-blue-50">
                <div className="w-14 h-14 bg-white rounded-2xl shadow-sm flex items-center justify-center text-3xl ring-4 ring-blue-50/50" role="img" aria-label="chart icon">
                    {visualSuggestion?.includes('Waterfall') && '📉'}
                    {visualSuggestion?.includes('Bar') && '📊'}
                    {visualSuggestion?.includes('Pie') && '🥧'}
                    {visualSuggestion?.includes('Process') && '➡️'}
                    {visualSuggestion?.includes('Tree') && '🌳'}
                    {visualSuggestion?.includes('Timeline') && '📅'}
                    {visualSuggestion?.includes('Key Takeaways') && '📝'}
                    {(!visualSuggestion || (!visualSuggestion.includes('Waterfall') && !visualSuggestion.includes('Bar') && !visualSuggestion.includes('Pie') && !visualSuggestion.includes('Process') && !visualSuggestion.includes('Tree') && !visualSuggestion.includes('Timeline') && !visualSuggestion.includes('Key Takeaways'))) && '💡'}
                </div>
